...
def fig_roc_curves(scores_dict: Dict[str, np.ndarray], proxy: np.ndarray, output_dir: Path):
    ...
    for model, scores in scores_dict.items():
        ...
        roc_auc = auc(fpr, tpr)
        ax.plot(fpr, tpr, color=MODEL_COLORS.get(model, "gray"),
...
def fig_score_distributions(scores_dict: Dict[str, np.ndarray], proxy: np.ndarray, output_dir: Path):
    _setup_style()
...
def fig_enrichment_curve(scores_dict: Dict[str, np.ndarray], proxy: np.ndarray, output_dir: Path):
    ...
    for model, scores in scores_dict.items():
        ...
        ax.plot(k_values * 100, efs, color=MODEL_COLORS.get(model, "gray"),
...
def fig_temporal_stability(results: Dict, output_dir: Path):
    ...
    for model in ["isolation_forest", "lof", "ocsvm"]:
        ...
        ax.plot(month_labels, aucs, "o-", color=MODEL_COLORS.get(model, "gray"),
                label=MODEL_NAMES.get(model, model))
...
def fig_posthoc_facility(posthoc: Dict, output_dir: Path):
    _setup_style()
    facs = posthoc.get("posthoc_analysis", {}).get("facility_concentration", {}).get("top_10_facilities", [])
    if not facs:
        return
...
def fig_posthoc_currency(posthoc: Dict, output_dir: Path):
    _setup_style()
    currs = posthoc.get("posthoc_analysis", {}).get("currency_concentration", {}).get("currencies_affected", [])
    if not currs:
        return
